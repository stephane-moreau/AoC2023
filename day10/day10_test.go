...
func findStart(landMap []string) position {
	for i, row := range landMap {
		j := strings.IndexByte(row, 'S')
		if j != -1 {
			return position{j, i}
		}
	}
...
func charAt(landMap []string, pos position) byte {
	if pos.y < 0 || pos.y > len(landMap) {
		return '.'
	}
	line := landMap[pos.y]
...
func move(landMap []string, path []position) []position {
	pathLength := len(path)
	cur := path[pathLength-1]
	switch charAt(landMap, cur) {
	case 'S':
		if pathLength != 1 {
			return nil
		}
		if next := charAt(landMap, position{cur.x, cur.y + 1}); next == 'L' || next == 'J' || next == '|' {
			return append(path, position{cur.x, cur.y + 1})
		}
		if next := charAt(landMap, position{cur.x, cur.y - 1}); next == '7' || next == 'F' || next == '|' {
			return append(path, position{cur.x, cur.y - 1})
		}
		if next := charAt(landMap, position{cur.x + 1, cur.y}); next == '7' || next == 'J' || next == '-' {
			return append(path, position{cur.x + 1, cur.y})
		}
		if next := charAt(landMap, position{cur.x - 1, cur.y}); next == 'F' || next == 'L' || next == '-' {
			return append(path, position{cur.x + 1, cur.y})
		}
		panic("Invalid start position")
	case '|':
		prevPos := path[pathLength-2]
		if prevPos.x != cur.x {
			panic("invalid move")
		}
		nextPos := position{cur.x, cur.y + cur.y - prevPos.y}
		if charAt(landMap, nextPos) == '.' {
			panic("invalid move")
		}
		return append(path, nextPos)
	case '-':
		prevPos := path[pathLength-2]
		if prevPos.y != cur.y {
			panic("invalid move")
		}
		nextPos := position{cur.x + cur.x - prevPos.x, cur.y}
		if charAt(landMap, nextPos) == '.' {
			panic("invalid move")
		}
		return append(path, nextPos)
	case 'L':
		...
		if prevPos.y == cur.y {
			nextPos = position{cur.x, cur.y - 1}
		} else if prevPos.x == cur.x {
			nextPos = position{cur.x + 1, cur.y}
		}
		if charAt(landMap, nextPos) == '.' {
			panic("invalid move")
		}
...
func computePath(landMap []string) []position {
	start := findStart(landMap)
	fmt.Printf("Start %v\n", start)
	path := make([]position, 0, 256)
	path = append(path, start)
	for {
		fmt.Printf("%c ", charAt(landMap, path[len(path)-1]))
		newPath := move(landMap, path)
		if newPath == nil {
			break
		}
		path = newPath
...
func isIn(point position, path []position) bool {
...
func sanitizeBorders(landMap [][]byte) {
	// Sanitize by rows
	for y := 0; y < len(landMap); y++ {
		line := landMap[y]
...
func dotCount(landMap [][]byte) int {
	i := 0
	for y := 0; y < len(landMap); y++ {
		line := []byte(landMap[y])
		for x := 0; x < len(line); x++ {
...
func expandSpaces(landMap [][]byte) bool {
	initDotCount := dotCount(landMap)
	for y := 0; y < len(landMap); y++ {
		line := landMap[y]
		for x := 0; x < len(line); x++ {
			if line[x] == ' ' {
	...
	return initDotCount != dotCount(landMap)
}

func TestDay10Phase1(t *testing.T) {
	// computePath(lightInput)
	// 4
	computePath(light2Input)
	//computePath(light3Input)
	// 8
	computePath(light4Input)
	// 6820
	computePath(largeInput)
}

func display(landMap [][]byte) {
	fmt.Printf("\n---------------------------------------------------------------------\n")
	for _, l := range landMap {
		fmt.Printf("%s\n", string(l))
	}
	fmt.Printf("\n---------------------------------------------------------------------\n")
}

func shrinkInput(byteInput [][]byte) [][]byte {
	newInput := make([][]byte, len(byteInput)/3)
	for i := range newInput {
		newInput[i] = make([]byte, len(byteInput[0])/3)
		for j := range newInput[i] {
			newInput[i][j] = byteInput[3*i+1][3*j+1]
		}
	}
	return newInput
}

func expandInput(byteInput [][]byte) [][]byte {
	newInput := make([][]byte, 3*len(byteInput))
	for i := range newInput {
		newInput[i] = make([]byte, 3*len(byteInput[0]))
		for j := range newInput[i] {
			newInput[i][j] = '.'
		}
	}
	for y := 0; y < len(byteInput); y++ {
		line := byteInput[y]
		for x := 0; x < len(line); x++ {
			newX := 3*x + 1
			newY := 3*y + 1
			newInput[newY][newX] = byteInput[y][x]
			switch byteInput[y][x] {
			case 'F':
				newInput[newY][newX+1] = '-'
				newInput[newY+1][newX] = '|'
			case '7':
				newInput[newY][newX-1] = '-'
				newInput[newY+1][newX] = '|'
			case 'L':
			...
			case '-':
				...
				newInput[newY][newX-1] = '-'
			case 'S':
				newInput[newY][newX-1] = 'S'
				newInput[newY-1][newX] = 'S'
				newInput[newY+1][newX] = 'S'
				newInput[newY][newX+1] = 'S'
			case ' ':
				newInput[newY-1][newX-1] = ' '
				newInput[newY][newX-1] = ' '
				newInput[newY+1][newX-1] = ' '
				newInput[newY-1][newX] = ' '
				newInput[newY][newX] = ' '
...
func phase2Test(input []string) {
	path := computePath(input)
	byteInput := make([][]byte, len(input))
	for i := range input {
	...
	sanitizeBorders(byteInput)
	display(byteInput)
	newByteInput := expandInput(byteInput)
	display(newByteInput)
	for expandSpaces(newByteInput) {
		display(newByteInput)
	}
	display(newByteInput)
	finalInput := shrinkInput(newByteInput)
	display(finalInput)
	fmt.Printf("\nDot count %d\n", dotCount(finalInput))
}
func TestDay10Phase2(t *testing.T) {
	// 4
	phase2Test(light2Input)
	phase2Test(light4Input)
	phase2Test(lightInput2)
	phase2Test(lightInput3)
	phase2Test(lightInput4)
	phase2Test(lightInput5)

	phase2Test(largeInput)
}
