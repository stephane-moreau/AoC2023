...
type subnet map[string]int
type nwk map[string]subnet

func parseInput(input []string) nwk {
	res := make(nwk)
	for _, l := range input {
		parts := strings.Split(l, ":")
		cnx := strings.Split(strings.TrimSpace(parts[1]), " ")
...
func expand(net subnet, subNetD int, full nwk, exclude subnet) {
	keys := []string{}
	for src := range net {
		keys = append(keys, src)
	}
	for _, src := range keys {
		for tgt, d := range full[src] {
			if d != 1 || exclude[tgt] != 0 {
				continue
			}
			if net[tgt] == 0 {
				net[tgt] = subNetD
...
func path(source, target string, res nwk, maxDepth int, visited map[string]bool, forbidenPaths []cnx) []cnx {
	if maxDepth-res[source][target] < 0 {
		return nil
	}
SearchPath:
	for tgt, d := range res[source] {
		if d != 1 || visited[tgt] {
			continue
		}
		for _, c := range forbidenPaths {
			if source == c.src && tgt == c.dst {
				continue SearchPath
			}
		}
		if tgt == target {
			return []cnx{{source, target}}
		}
		visited[tgt] = true
		next := path(tgt, target, res, maxDepth-1, visited, forbidenPaths)
		delete(visited, tgt)
		if next != nil {
			return append([]cnx{{source, tgt}}, next...)
		}
	}
	return nil
}

func buildSubNet(s string, res nwk, forbiden []cnx) subnet {
	net := subnet{s: 1}
	count := 0
	for count != len(net) {
		count = len(net)
		...
		SubNetSearch:
			for t, d := range res[s] {
				...
				for _, c := range forbiden {
					if c.src == s && c.dst == t {
						continue SubNetSearch
					}
				}
				net[t] = 1
			}
...
func splitNet(res nwk) (subnet, subnet) {
	fmt.Printf("%v\n", res)
	i := 0
	dMax := 0
	for src, cnx := range res {
		i++
		d := 1
		for len(cnx) != len(res)-1 {
			d++
			expand(cnx, d, res, subnet{src: 1})
		}
		if d > dMax {
			dMax = d
		}
		if (i % 100) == 0 {
			fmt.Printf("Max dist : %d (i= %d)\n", dMax, i)
		}
	}
	fmt.Printf("Max dist : %d (i= %d)\n", dMax, i)
	// finds max distance between source and target
	var farthestNodes []cnx
	var distance int
	for src, v := range res {
		for tgt, d := range v {
			if d > distance {
				distance = d
				farthestNodes = []cnx{{src, tgt}}
			} else if d == distance {
				farthestNodes = append(farthestNodes, cnx{src, tgt})
			}
		}
	}
	fmt.Printf("%s : %d\n", farthestNodes, distance)

	cnxCut := []cnx{}
	for len(cnxCut) < 6 {

		paths := make([][]cnx, 0, len(farthestNodes))
		for _, c := range farthestNodes {
			for newSrc, d := range res[c.src] {
				if d != 1 {
					continue
				}
				for newDst, d := range res[c.dst] {
					if d != 1 {
						continue
					}
					paths = append(paths, path(newSrc, newDst, res, distance, map[string]bool{c.src: true}, cnxCut))
				}
			}
		}

		freq := make(map[cnx]int)
		for _, p := range paths {
		Path:
			for _, c := range p {
				for _, n := range farthestNodes {
					if c.src == n.src || c.dst == n.dst {
						// skip connection from or to farthest node
						continue Path
					}
				}
				if freq[cnx{c.dst, c.src}] != 0 {
					freq[cnx{c.dst, c.src}]++
				} else {
					freq[c]++
				}
			}
		}
		// search for 3 max used connection leading to a split
		fMax := 0
		var newCuts []cnx
		for c, f := range freq {
			if f > fMax {
		...
		for _, c := range newCuts {
			cnxCut = append(cnxCut, c, cnx{c.dst, c.src})
		}
	}

	net1 := buildSubNet(farthestNodes[0].src, res, cnxCut)
	net2 := buildSubNet(farthestNodes[0].dst, res, cnxCut)
	return net1, net2
}

func TestDay25Phase1(t *testing.T) {
	n1, n2 := splitNet(parseInput(lightInput))
	if len(n1)*len(n2) != 54 {
		t.Fail()
	}

	n1, n2 = splitNet(parseInput(largeInput))
	// 555702 NOK
	// 555780
	// 555930
	// 556002 // to high
	if len(n1)*len(n2) != 555856 {
		t.Fail()
	}
}
