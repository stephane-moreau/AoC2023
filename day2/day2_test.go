...
func TestDay2Phase1(t *testing.T) {
	dataToTest := largeInput
	gameSum := 0
InputInspection:
	for _, l := range dataToTest {
		colonIndex := strings.Index(l, ":")
		game := l[:colonIndex]
		gameNum, err := strconv.Atoi(game[strings.Index(game, " ")+1:])
		if err != nil {
			panic(err)
		}
		sets := strings.Split(l[colonIndex+1:], ";")
		for _, set := range sets {
			for _, s := range strings.Split(strings.TrimSpace(set), ",") {
				numAndColor := strings.Split(strings.TrimSpace(s), " ")
				num, err := strconv.Atoi(numAndColor[0])
				if err != nil {
					panic(err)
				}
				color := strings.TrimSpace(numAndColor[1])
				if bagNum, exists := gameBag[color]; !exists || bagNum < num {
					continue InputInspection
				}
			}
		}
		gameSum += gameNum
...
func TestDay2Phase2(t *testing.T) {
	dataToTest := largeInput
	gameSum := 0
	for _, l := range dataToTest {
		colonIndex := strings.Index(l, ":")
		game := l[:colonIndex]
		_, err := strconv.Atoi(game[strings.Index(game, " ")+1:])
		if err != nil {
			panic(err)
		}
