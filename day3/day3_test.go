...
type gear []int
type gears map[pos]gear

func isUsablePiece(lines []string, lineNdx, startNum, endNum int) (bool, *pos) {
	var prevLine, nextLine string
	line := lines[lineNdx]
	if lineNdx > 0 {
		prevLine = lines[lineNdx-1]
	}
	if lineNdx < len(lines)-1 {
		nextLine = lines[lineNdx+1]
	}
	...
	for i := startNum; i < len(prevLine) && i < endNum; i++ {
		if isSymbol(prevLine[i]) {
			if prevLine[i] == '*' {
				gear := pos{lineNdx - 1, i}
				return true, &gear
			}
			return true, nil
		}
	}
	for i := startNum; i < len(nextLine) && i < endNum; i++ {
	...
	if line[startNum] == '*' {
		return true, &pos{lineNdx, startNum}
	}
	if line[endNum-1] == '*' {
		return true, &pos{lineNdx, endNum - 1}
	}
	return isSymbol(line[startNum]) || isSymbol(line[endNum-1]), nil
}

func compute(dataToTest []string) {
	gameSum := 0
	foundGears := make(gears)
	for i := 0; i < len(dataToTest); i++ {
		line := dataToTest[i]
		startNum := -1
		endNum := -1
		for j := 0; j < len(line); j++ {
			...
			if startNum != -1 && (j == len(line)-1 || !isDigit(line[j+1])) {
				n, err := strconv.Atoi(line[startNum : endNum+1])
				if err != nil {
					panic(err)
				}
				usable, pos := isUsablePiece(dataToTest, i, startNum, endNum+1)
				if pos != nil {
					foundGears[*pos] = append(foundGears[*pos], n)
				}
				if usable {
					gameSum += n
				}
	...
	fmt.Printf("sum of pieces: %d\n", gameSum)
	engineRatio := 0
	for _, wheels := range foundGears {
		if len(wheels) == 2 {
			engineRatio += wheels[0] * wheels[1]
		}
	}
	fmt.Printf("sum of ratios: %d\n", engineRatio)
}
